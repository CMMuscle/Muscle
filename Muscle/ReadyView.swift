...
struct ReadyView: View {
    @StateObject var readyViewModel: ReadyViewModel = .shared
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 30){
                ...
                Button {
                    if readyViewModel.startTime == false {
                        readyViewModel.startTime = true
                        //STARTボタンを押した際の処理
                        readyViewModel.startTimer()
                    }
                } label: {
                    // 楕円形の描画
                    Text("START")
...
struct ReadyView_Previews: PreviewProvider {
    static var previews: some View {
        ReadyView(menuViewModel: MenuViewModel(), image: "プランク")
    }
}
